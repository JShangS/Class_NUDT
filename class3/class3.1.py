...
###第二个层###################
L21, L22 = add_layer(L11, L12, 500, 10, tf.nn.relu)
###############计算Ew和预测##################
Ew = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(L21, L22)), 1))
pred = tf.nn.sigmoid(Ew)
############loss#############
...
with tf.Session() as sess:
    ...
    for itera in range(iter):
        ...
        if np.isnan(index0).sum():
            continue
        y__s = np.append(y_s[index0], y_s[0:32, :], axis=0)
        x__1 = np.append(x_1[index0, :], x_1[0:32, :], axis=0)
        x__2 = np.append(x_2[index0, :], x_2[0:32, :], axis=0)
