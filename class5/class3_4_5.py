...
##用class3的程序来看看tensorboard，改了下class3的程序，使用变量复用
    ...
    with tf.name_scope(name):
        ...
        with tf.variable_scope(name_var):
            Weights = tf.get_variable(
                'w', initializer=tf.random_normal(
                    [in_size, out_size]))  #random_normal#zeros
            biases = tf.get_variable(
                'b', initializer=tf.random_normal([1, out_size]))
            Wx_plus_b1 = tf.matmul(inputs1, Weights) + biases
        with tf.variable_scope(name_var, reuse=True):
        ...
        tf.summary.histogram("activations1", outputs1)
        tf.summary.histogram("activations2", outputs2)
        #####################################
...
x_image1 = tf.reshape(x1, [-1, 28, 28, 1])
x_image2 = tf.reshape(x2, [-1, 28, 28, 1])
tf.summary.image('input1', x_image1)
tf.summary.image('input2', x_image2)

###第一层###################
L11, L12 = add_layer('fc1', x1, x2, 784, 500, tf.nn.leaky_relu)  #tf.nn.sigmoid
###第二个层###################
L21, L22 = add_layer(
    'fc2', L11, L12, 500, 10,
...
writer = tf.summary.FileWriter(LOGDIR + 'JS_board1S/')
...
with tf.Session() as sess:
    ...
    for itera in range(iter):
        ...
        ##############每隔50次写入一次####################
        if itera % 50 == 0:
            s = sess.run(merge_all, feed_dict={x1: xs_1, x2: xs_2, y: y_s})
